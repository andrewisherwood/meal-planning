...
export function ShoppingModal({
  ...
  const SHORTCUT_SETUP_KEY = "remindersShortcutSetupSeen";
  const SHORTCUT_URL = "https://www.icloud.com/shortcuts/11b5b64390414b6993c842802ad93149";
  ...
  // Send to iOS Reminders via Shortcut
  const handleSendToReminders = async () => {
    // Check if user has seen setup instructions
    const hasSeenSetup = localStorage.getItem(SHORTCUT_SETUP_KEY) === "true";
    ...
    // Copy clean list to clipboard (Shortcut reads from clipboard)
    ...
    // Open the Shortcut
    window.location.href = "shortcuts://run-shortcut?name=Add%20to%20Shopping";
  };

  // Called when user confirms they've installed the Shortcut
  const handleSetupComplete = () => {
    localStorage.setItem(SHORTCUT_SETUP_KEY, "true");
    setShowSetupModal(false);
  };
  ...
  return (
    <>
    {/* Setup Modal for first-time Reminders users */}
    <Dialog open={showSetupModal} onOpenChange={(isOpen) => !isOpen && setShowSetupModal(false)}>
      <DialogContent className="bg-white rounded-3xl p-6 max-w-sm w-[90vw] shadow-2xl border-0">
        <DialogHeader>
        ...
        <div className="mt-4 space-y-4">
          <p className="text-sm text-text-secondary">
            To send items to Reminders, you need to install a free Apple Shortcut first.
          </p>
          <p className="text-sm text-text-secondary">
            The Shortcut reads your shopping list from the clipboard and creates individual reminders in your Shopping list.
          </p>
          <a
            href={SHORTCUT_URL}
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl bg-blue-500 text-white font-medium hover:bg-blue-600 transition-colors"
          >
            <svg
          ...
          <button
            ...
            className="w-full px-4 py-2.5 rounded-xl border border-border text-text-primary font-medium hover:bg-surface-muted transition-colors cursor-pointer"
          >
            I&apos;ve installed it
          </button>
        </div>
